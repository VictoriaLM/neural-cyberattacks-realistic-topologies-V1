...
//Pruebas con Simulate()
void
nest::SimulationManager::prueba_()
{
  std::ostringstream os;
  LOG( M_INFO, "SimulationManager::prueba_", os.str() );
  const thread tid = kernel().vp_manager.get_thread_id();
              ...
              ++i )
  {
    ...
    /*std::cout << node->get_node_id() << std::endl;
    ...
    double vm_value = my_dict["V_m"];
    std::cout << "Voltage: " << vm_value << std::endl;

    if(node->get_node_id() == 2)
    {
      my_dict["V_m"] = -20;
      std::cout << "New voltage node 2: " <<  my_dict["V_m"] << std::endl;
    }
   ...
   /*if(node->get_node_id() == 1)
    {
      node->set_node_id_(3);
      std::cout << "Cambiado: " << node->get_node_id() << std::endl;
    }*/ 
     
  std::cout << "Hola, soy la función de prueba de ataque";

}

//Attack's dictionaries
void
nest::SimulationManager::load_dictionary_()
{
  ...
  if(attack_file.is_open() && FLO_file.is_open() && JAM_file.is_open())
  {
    ...
    while(getline(attack_file, text1))
    {
   
      std::string key,value;
      key = text1.substr(0,text1.find(':'));
      value = text1.substr(text1.find(":") + 1);
      type_attack[key] = value;
    }

    while(getline(FLO_file, text2))
    {
      std::string key,value;

      key = text2.substr(0,text2.find(':'));
      value = text2.substr(text2.find(":") + 1);
      FLO_dict[key] = value;
    }

    while(getline(JAM_file, text3))
    {
      std::string key,value;

      key = text3.substr(0,text3.find(':'));
      value = text3.substr(text3.find(":") + 1);
      JAM_dict[key] = value;
    }
  }
  attack_file.close();
...
nest::SimulationManager::FLO_attack_()
{
  ...
  for (int i = 0; i < 450; i++) {
    list_random.push_back(i);
  }

  for(int i = 0; i < std::stoi(FLO_dict["n_neurons"]); i++)
  {
    ...
    {
      Node* node = i->get_node();
      dict = kernel().node_manager.get_status(node->get_node_id());
      Dictionary my_dict = *dict;
      std::string model = my_dict["model"];

      if(!my_dict["V_m"].empty() && std::stoi(FLO_dict["n_neurons"]) < 450)
      {
        int value = node->get_node_id();
        auto it = std::find(list_random.begin(), list_random.end(), random_num);
        if(value == random_num && it != list_random.end())
        {
          list_random.erase(it);

          if(std::stoi(FLO_dict["v_increment"]) == 551)
          {
            my_dict["V_m"] = my_dict["V_th"]; 
            node->set_status_base(my_dict);
          }else{
            float value_VM = my_dict["V_m"].operator double() + std::stoi(FLO_dict["v_increment"]);
            my_dict["V_m"] = value_VM; 
            node->set_status_base(my_dict);
          }
        }
      }else if(!my_dict["V_m"].empty() && std::stoi(FLO_dict["n_neurons"]) == 450)
      {
        if(std::stoi(FLO_dict["v_increment"]) == 551)
        {
          my_dict["V_m"] = my_dict["V_th"]; 
...
nest::SimulationManager::JAM_attack_()
{
  ...
  for(int i = 0; i < std::stoi(JAM_dict["n_neurons"]); i++)
  {
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<> dis(1, 300);
              ...
              ++i )
    {
      ...
      if(!my_dict["V_m"].empty() && std::stoi(JAM_dict["n_neurons"]) < 450)
      {
        int value = node->get_node_id();
        auto it = std::find(list_random.begin(), list_random.end(), random_num);
        if(value == random_num && it != list_random.end())
        {
          ...
          my_dict["V_m"] = my_dict["V_reset"]; 
          node->set_status_base(my_dict);
        }
      }else if(!my_dict["V_m"].empty() && std::stoi(JAM_dict["n_neurons"]) == 450)
      {
          my_dict["V_m"] = my_dict["V_reset"]; 
          node->set_status_base(my_dict);
      }
      ...
      //Check
       const thread tid = kernel().vp_manager.get_thread_id();
          ...
          i != kernel().node_manager.get_local_nodes( tid ).end(); ++i )
        {
          ...
          std::cout << clock_.get_ms() << " | Nodo: " << node->get_node_id() << " | Model: " << my_dict["model"] <<" | V_m: " << my_dict["V_m"] << " | V_th: " << my_dict["V_th"] << std::endl;
        } 
      }
// end of master section, all threads have to synchronize at this point
#pragma omp barrier
